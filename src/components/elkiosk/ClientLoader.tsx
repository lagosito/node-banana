 ...
 *
 * Fetches Brand DNA from Airtable and injects it into the
 * workflow's "Brand Context" prompt node automatically.
 *
 * Usage: placed in the Header alongside the existing controls.
 */
...
// Tier workflows — imported at runtime via fetch
...
// Builds the Brand DNA prompt string injected into the "Brand Context" node
...
export function ClientLoader() {
  ...
  useEffect(() => {
    ...
    fetch("/api/client")
      ...
      .catch(() => {
        setError("Failed to load clients from Airtable");
        setLoadingClients(false);
      });
  }, [isOpen]);

  // Inject Brand DNA into current workflow nodes
  const injectBrandDNA = useCallback(
    (client: ClientBrandDNA) => {
      const brandPrompt = buildBrandPrompt(client);

      // Find the Brand Context prompt node (variableName = "brand_context")
      const brandNode = nodes.find(
        (n) =>
          n.type === "prompt" &&
          (n.data as { variableName?: string }).variableName === "brand_context"
      );

      if (brandNode) {
        updateNodeData(brandNode.id, { prompt: brandPrompt } as never);
      } else {
        console.warn("[ElKiosk] No 'brand_context' prompt node found in current workflow");
      }
    },
    [nodes, updateNodeData]
  );

  // Load a tier workflow template first, then inject brand DNA
    ...
    try {
      ...
      if (workflowUrl) {
        ...
        if (wRes.ok) {
          ...
          // 3. Inject after workflow loads (nodes need a tick to populate)
          setTimeout(() => {
            injectBrandDNA(client);
          }, 100);
        }
      } else {
        // No template — just inject into existing workflow
        injectBrandDNA(client);
      }
  ...
  // Also expose "inject only" (without reloading workflow)
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-3 py-2 rounded-md text-sm
                         ...
                         disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              title="Inject Brand DNA into current workflow without changing template"
            >
              DNA only
            </button>
          </div>
          ...
          {currentClient && (
            <div className="mt-4 p-3 bg-zinc-800 rounded-lg text-xs space-y-1 text-zinc-300">
              ...
              <div className="flex gap-2 flex-wrap">
                {[
                  currentClient.primaryColor,
                  currentClient.secondaryColor,
                  currentClient.accentColor,
              ...
              {currentClient.toneTags && (
                <div><span className="text-zinc-500">Tone:</span> {currentClient.toneTags}</div>
              )}
              {currentClient.displayFont && (
                <div><span className="text-zinc-500">Font:</span> {currentClient.displayFont}</div>
              )}
            </div>
          )}
        </div>
